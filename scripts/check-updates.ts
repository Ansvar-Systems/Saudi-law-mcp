#!/usr/bin/env tsx
/**
 * Saudi Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://laws.boe.gov.sa';
const PORTAL_NAME = 'Bureau of Experts (Saudi Arabia)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/saudi-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Saudi Law MCP — Data Freshness Check');
