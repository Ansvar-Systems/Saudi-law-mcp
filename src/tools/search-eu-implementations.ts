/**
 * search_eu_implementations — Search EU directives/regulations referenced by Saudi legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_saudi_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as saudi_statute_count
  ...
  if (input.has_saudi_implementation) {
    sql += ' HAVING saudi_statute_count > 0';
  }
