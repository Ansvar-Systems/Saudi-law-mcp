/**
 * Tool registry for Saudi Law MCP Server.
...
import { getSaudiImplementations, type GetSaudiImplementationsInput } from './get-saudi-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Umm al-Qura (Royal Court Official Gazette). ' +
    ...
    'Call this FIRST when you need to understand what Saudi legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Saudi statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Saudi statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Saudi legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Saudi statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Saudi?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Saudi legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Saudi statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Saudi statute references or aligns with. ' +
      'Saudi laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Saudi statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_saudi_implementations',
    description:
      'Find all Saudi statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Saudi statutes. ' +
      'Saudi may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Saudi legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_saudi_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Saudi legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Saudi statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Saudi statute or provision. ' +
      ...
      'Saudi is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Saudi statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_saudi_implementations':
          result = await getSaudiImplementations(db, args as unknown as GetSaudiImplementationsInput);
          break;
