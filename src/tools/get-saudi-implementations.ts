/**
 * get_saudi_implementations — Find Saudi statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetSaudiImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface SaudiImplementationResult {
  document_id: string;
...
export async function getSaudiImplementations(
  db: InstanceType<typeof Database>,
  input: GetSaudiImplementationsInput,
): Promise<ToolResponse<SaudiImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as SaudiImplementationResult[];
